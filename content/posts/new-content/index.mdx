...
To use certbot for obtaining certificate you can use `certonly` command. And the following arguments can be used based on your condition

* `--webroot` argument can be added if you already have running server and you'd prefer not to stop it. By using this argument you need to specify top-level directory ("web root") containing the files served by your server using `--webroot-path` argument (or `-w` for short)
* `--standalone` argument can be added if your application are not running. By using this argument, the certbot will start their own server and listening on port 80 in order to perform domain validation.
* `--manual` argument can be added if you want to obtain certificate manually.
